...
5.  Opta Para especificar el número de posiciones decimales que muestra la etiqueta, escriba, `#PERCENT{Pn}` donde *n* es el número de posiciones decimales que se van a mostrar. Por ejemplo, para no mostrar ninguna posición decimal, escriba `#PERCENT{P0}` .  
