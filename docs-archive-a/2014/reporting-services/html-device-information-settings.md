...
> [!IMPORTANT]  
>  La configuración de información de dispositivo que aparece en la siguiente tabla con **(\*)** está en desuso y no se debe usar en nuevas aplicaciones. Para obtener más información, consulte [características desusadas en SQL Server Reporting Services en SQL Server 2014](deprecated-features-in-sql-server-reporting-services-ssrs.md).  
...
|`LinkTarget`|Destino para los hipervínculos en el informe. Puede establecer como destino una ventana o un marco proporcionando el nombre de la ventana, como `LinkTarget` = *window_name*, o bien puede establecer como destino una nueva ventana mediante `LinkTarget` = _blank. Otros nombres de destino válidos incluyen _self, _parent y _top.|  
