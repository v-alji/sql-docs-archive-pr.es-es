...
4.  En la **expresión** presentada, haga clic en en `ProductID` la lista desplegable. Es la columna a la que se aplica el filtro.  
  
5.  Haga clic en el **=** operador igual () en la lista desplegable **operador** .  
  
6.  Haga clic en el botón expresión situado junto al campo **valor** , haga clic en **parámetros** en el área **categoría** y, a continuación, haga doble clic `productid` en el área **valores** . El campo **Establecer expresión para: Valor** ahora debe contener una expresión similar a **=Parameters!productid.Value**.  
...
2.  Cree una nueva función que acepte un parámetro, `productid`, de tipo Integer y devuelva un objeto `datatable` y haga lo siguiente.  
    ...
    1.  Crea una instancia del conjunto de `DataSet2` datos,, que se creó en el paso 2 de la [Lección 4: definir una conexión de datos y una tabla de datos para el informe secundario](lesson-4-define-a-data-connection-and-data-table-for-child-report.md).  
...
5.  Complete el controlador de eventos. Debe incluir la funcionalidad siguiente.  
    ...
    5.  Enlazar el `PurchaseOrderDetail` DataTable con el origen de datos correspondiente del informe.  
