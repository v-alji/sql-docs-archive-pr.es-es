---
title: Tipo definido por el usuario | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "87746456"
---
# <a name="user-defined-type"></a><span data-ttu-id="c559b-102">Tipo definido por el usuario</span><span class="sxs-lookup"><span data-stu-id="c559b-102">User Defined Type</span></span>
  <span data-ttu-id="c559b-103">El ejemplo Tipo de datos definido por el usuario muestra la creación y el uso de un tipo de datos simple definido por el usuario desde Transact-SQL y desde una aplicación cliente mediante `System.Data.SqlClient`.</span><span class="sxs-lookup"><span data-stu-id="c559b-103">The User Defined Data Type sample shows the creation and use of a simple user-defined data type from both Transact-SQL and a client application using `System.Data.SqlClient`.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="c559b-104">Requisitos previos</span><span class="sxs-lookup"><span data-stu-id="c559b-104">Prerequisites</span></span>  
 <span data-ttu-id="c559b-105">Para crear y ejecutar este proyecto se debe instalar el siguiente software:</span><span class="sxs-lookup"><span data-stu-id="c559b-105">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="c559b-106">o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="c559b-106">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="c559b-107">Puede obtener [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express de forma gratuita desde el [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sitio web [de documentación y ejemplos de](https://www.microsoft.com/download/details.aspx?id=42299)Express.</span><span class="sxs-lookup"><span data-stu-id="c559b-107">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/download/details.aspx?id=42299)</span></span>  
  
-   <span data-ttu-id="c559b-108">La base de datos de AdventureWorks que está disponible en el [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sitio web [para desarrolladores de](https://archive.codeplex.com/?p=SqlServerSamples).</span><span class="sxs-lookup"><span data-stu-id="c559b-108">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://archive.codeplex.com/?p=SqlServerSamples)</span></span>  
  
-   <span data-ttu-id="c559b-109">.NET Framework SDK 2.0 o posterior, o Microsoft Visual Studio 2005 o posterior.</span><span class="sxs-lookup"><span data-stu-id="c559b-109">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="c559b-110">Puede obtener .NET Framework SDK de forma gratuita.</span><span class="sxs-lookup"><span data-stu-id="c559b-110">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="c559b-111">Además, se deben cumplir las siguientes condiciones:</span><span class="sxs-lookup"><span data-stu-id="c559b-111">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="c559b-112">La instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que está usando debe tener habilitada la integración con CLR.</span><span class="sxs-lookup"><span data-stu-id="c559b-112">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="c559b-113">Para habilitar la integración con CLR, siga estos pasos:</span><span class="sxs-lookup"><span data-stu-id="c559b-113">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="c559b-114">Habilitar la integración con CLR</span><span class="sxs-lookup"><span data-stu-id="c559b-114">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="c559b-115">Ejecute los siguientes comandos [!INCLUDE[tsql](../../includes/tsql-md.md)] :</span><span class="sxs-lookup"><span data-stu-id="c559b-115">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="c559b-116">Para habilitar CLR, debe tener el permiso de nivel de servidor `ALTER SETTINGS`, que se concede implícitamente a los miembros de los roles fijos de servidor `sysadmin` y `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="c559b-116">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="c559b-117">La base de datos de AdventureWorks debe estar instalada en la instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que está usando.</span><span class="sxs-lookup"><span data-stu-id="c559b-117">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="c559b-118">Si no es administrador de la instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que está usando, debe hacer que un administrador le conceda el permiso **CreateAssembly** para completar la instalación.</span><span class="sxs-lookup"><span data-stu-id="c559b-118">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="c559b-119">Generar el ejemplo</span><span class="sxs-lookup"><span data-stu-id="c559b-119">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="c559b-120">Cree y ejecute el ejemplo utilizando las siguientes instrucciones:</span><span class="sxs-lookup"><span data-stu-id="c559b-120">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="c559b-121">Abra un símbolo del sistema de Visual Studio o de .NET Framework.</span><span class="sxs-lookup"><span data-stu-id="c559b-121">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="c559b-122">Si es necesario, cree un directorio para el ejemplo.</span><span class="sxs-lookup"><span data-stu-id="c559b-122">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="c559b-123">Para este ejemplo, utilizaremos C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="c559b-123">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="c559b-124">En c:\MySample, cree `ComplexNumber.vb` (para el ejemplo de Visual Basic) o `ComplexNumber.cs` (para el ejemplo de C#) y copie el código muestra de Visual Basic o de C# que corresponda (más abajo) en el archivo.</span><span class="sxs-lookup"><span data-stu-id="c559b-124">In c:\MySample, create `ComplexNumber.vb` (for the Visual Basic sample) or `ComplexNumber.cs` (for the C# sample) and copy the appropriate Visual Basic or C# sample code (below) into the file.</span></span>  
  
4.  <span data-ttu-id="c559b-125">Compile el código muestra desde el símbolo del sistema ejecutando uno de los comandos siguientes, dependiendo de su opción de lenguaje.</span><span class="sxs-lookup"><span data-stu-id="c559b-125">Compile the sample code from the command line prompt by executing one of the following, depending on your choice of language.</span></span>  
...
6.  <span data-ttu-id="c559b-126">Copie el código de instalación de [!INCLUDE[tsql](../../includes/tsql-md.md)] en un archivo y guárdelo como `Install.sql` en el directorio de ejemplo.</span><span class="sxs-lookup"><span data-stu-id="c559b-126">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
7.  <span data-ttu-id="c559b-127">Si el ejemplo está instalado en un directorio distinto de `C:\MySample\`, edite el archivo `Install.sql` del archivo como se indica para señalar esa ubicación.</span><span class="sxs-lookup"><span data-stu-id="c559b-127">If the sample is installed in a directory other then `C:\MySample\`, edit the file `Install.sql` as indicated to point to that location.</span></span>  
  
8.  <span data-ttu-id="c559b-128">Implemente el ensamblado y el procedimiento almacenado ejecutando</span><span class="sxs-lookup"><span data-stu-id="c559b-128">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql`  
  
9. <span data-ttu-id="c559b-129">Copie el script de comando de prueba de [!INCLUDE[tsql](../../includes/tsql-md.md)] en un archivo y guárdelo como `test.sql` en el directorio de ejemplo.</span><span class="sxs-lookup"><span data-stu-id="c559b-129">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
10. <span data-ttu-id="c559b-130">Ejecute el script de prueba con el siguiente comando</span><span class="sxs-lookup"><span data-stu-id="c559b-130">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
11. <span data-ttu-id="c559b-131">Para ejecutar el programa cliente de ejemplo (C# solamente):</span><span class="sxs-lookup"><span data-stu-id="c559b-131">To run the client example program (C# only):</span></span>  
  
    -   <span data-ttu-id="c559b-132">En c:\MySample, cree el archivo `Program.cs` y copie el código fuente en el archivo.</span><span class="sxs-lookup"><span data-stu-id="c559b-132">In c:\MySample, create `Program.cs` and copy the source code into the file.</span></span>  
  
    -   <span data-ttu-id="c559b-133">Ejecut`: Csc /reference:ComplexNumber.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.Data.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.XML.dll /out:Client.exe /target:exe Program.cs`</span><span class="sxs-lookup"><span data-stu-id="c559b-133">Execute`: Csc /reference:ComplexNumber.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.Data.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.XML.dll /out:Client.exe /target:exe Program.cs`</span></span>  
  
    -   <span data-ttu-id="c559b-134">Pruebe el ejemplo ejecutando: `Client AdventureWorks`</span><span class="sxs-lookup"><span data-stu-id="c559b-134">Test by executing: `Client AdventureWorks`</span></span>  
  
12. <span data-ttu-id="c559b-135">Copie el script de limpieza de [!INCLUDE[tsql](../../includes/tsql-md.md)] en un archivo y guárdelo como `cleanup.sql` en el directorio de ejemplo.</span><span class="sxs-lookup"><span data-stu-id="c559b-135">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
13. <span data-ttu-id="c559b-136">Ejecute el script con el siguiente comando</span><span class="sxs-lookup"><span data-stu-id="c559b-136">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="c559b-137">Código de ejemplo</span><span class="sxs-lookup"><span data-stu-id="c559b-137">Sample Code</span></span>  
 <span data-ttu-id="c559b-138">A continuación se muestran las listas de código para este ejemplo.</span><span class="sxs-lookup"><span data-stu-id="c559b-138">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="c559b-139">C#</span><span class="sxs-lookup"><span data-stu-id="c559b-139">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="c559b-140">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="c559b-140">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="c559b-141">Este es el script (`Install.sql`) de instalación de [!INCLUDE[tsql](../../includes/tsql-md.md)], que implementa el ensamblado y crea el UDT en la base de datos.</span><span class="sxs-lookup"><span data-stu-id="c559b-141">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the UDT in the database.</span></span>  
...
```  
 ...
 <span data-ttu-id="c559b-142">Este es `test.sql`, que prueba el ejemplo ejerciendo el tipo.</span><span class="sxs-lookup"><span data-stu-id="c559b-142">This is `test.sql`, which tests the sample by exercising the type.</span></span>  
...
```  
 ...
 <span data-ttu-id="c559b-143">El siguiente código de [!INCLUDE[tsql](../../includes/tsql-md.md)] quita el ensamblado y el tipo de la base de datos.</span><span class="sxs-lookup"><span data-stu-id="c559b-143">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly and type from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="c559b-144">Consulte también</span><span class="sxs-lookup"><span data-stu-id="c559b-144">See Also</span></span>  
 [<span data-ttu-id="c559b-145">Escenarios de uso y ejemplos para la integración de Common Language Runtime &#40;CLR&#41;</span><span class="sxs-lookup"><span data-stu-id="c559b-145">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
