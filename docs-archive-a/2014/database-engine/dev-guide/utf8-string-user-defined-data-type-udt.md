---
title: Tipo de datos definido por el usuario (UDT) de cadena UTF8 | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "87746450"
---
# <a name="utf8-string-user-defined-data-type-udt"></a><span data-ttu-id="99b8e-102">Tipo de datos definido por el usuario (UDT) en cadenas UTF8</span><span class="sxs-lookup"><span data-stu-id="99b8e-102">UTF8 String User-Defined Data Type (UDT)</span></span>
  <span data-ttu-id="99b8e-103">El ejemplo UTF8String para SQL Server muestra la implementación de un tipo de datos definido por el usuario.</span><span class="sxs-lookup"><span data-stu-id="99b8e-103">The UTF8String sample for SQL Server demonstrates the implementation of a user-defined data type.</span></span> <span data-ttu-id="99b8e-104">Este ejemplo muestra la implementación de un tipo de datos definido por el usuario UTF8 que amplía el sistema de tipos de la base de datos para proporcionar almacenamiento para valores de codificación UTF8.</span><span class="sxs-lookup"><span data-stu-id="99b8e-104">This sample shows the implementation of a UTF8 user-defined data type that extends the type system of the database to provide storage for UTF8-encoded values.</span></span> <span data-ttu-id="99b8e-105">Este tipo también implementa código para convertir cadenas Unicode al formato UTF8 y desde este formato.</span><span class="sxs-lookup"><span data-stu-id="99b8e-105">This type also implements code to convert Unicode strings to and from UTF8.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="99b8e-106">Requisitos previos</span><span class="sxs-lookup"><span data-stu-id="99b8e-106">Prerequisites</span></span>  
 <span data-ttu-id="99b8e-107">Para crear y ejecutar este proyecto se debe instalar el siguiente software:</span><span class="sxs-lookup"><span data-stu-id="99b8e-107">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="99b8e-108">o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="99b8e-108">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="99b8e-109">Puede obtener [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express de forma gratuita desde el [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sitio web [de documentación y ejemplos de](https://www.microsoft.com/sql-server/sql-server-editions-express)Express.</span><span class="sxs-lookup"><span data-stu-id="99b8e-109">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="99b8e-110">La base de datos de AdventureWorks que está disponible en el [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sitio web [para desarrolladores de](https://go.microsoft.com/fwlink/?linkid=62796).</span><span class="sxs-lookup"><span data-stu-id="99b8e-110">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="99b8e-111">.NET Framework SDK 2.0 o posterior, o Microsoft Visual Studio 2005 o posterior.</span><span class="sxs-lookup"><span data-stu-id="99b8e-111">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="99b8e-112">Puede obtener .NET Framework SDK de forma gratuita.</span><span class="sxs-lookup"><span data-stu-id="99b8e-112">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="99b8e-113">Además, se deben cumplir las siguientes condiciones:</span><span class="sxs-lookup"><span data-stu-id="99b8e-113">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="99b8e-114">La instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que está usando debe tener habilitada la integración con CLR.</span><span class="sxs-lookup"><span data-stu-id="99b8e-114">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="99b8e-115">Para habilitar la integración con CLR, siga estos pasos:</span><span class="sxs-lookup"><span data-stu-id="99b8e-115">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="99b8e-116">Habilitar la integración con CLR</span><span class="sxs-lookup"><span data-stu-id="99b8e-116">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="99b8e-117">Ejecute los siguientes comandos [!INCLUDE[tsql](../../includes/tsql-md.md)] :</span><span class="sxs-lookup"><span data-stu-id="99b8e-117">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="99b8e-118">Para habilitar CLR, debe tener el permiso de nivel de servidor `ALTER SETTINGS`, que se concede implícitamente a los miembros de los roles fijos de servidor `sysadmin` y `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="99b8e-118">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="99b8e-119">La base de datos de AdventureWorks debe estar instalada en la instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que está usando.</span><span class="sxs-lookup"><span data-stu-id="99b8e-119">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="99b8e-120">Si no es administrador de la instancia de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que está usando, debe hacer que un administrador le conceda el permiso **CreateAssembly** para completar la instalación.</span><span class="sxs-lookup"><span data-stu-id="99b8e-120">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="99b8e-121">Generar el ejemplo</span><span class="sxs-lookup"><span data-stu-id="99b8e-121">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="99b8e-122">Cree y ejecute el ejemplo utilizando las siguientes instrucciones:</span><span class="sxs-lookup"><span data-stu-id="99b8e-122">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="99b8e-123">Abra un símbolo del sistema de Visual Studio o de .NET Framework.</span><span class="sxs-lookup"><span data-stu-id="99b8e-123">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="99b8e-124">Si es necesario, cree un directorio para el ejemplo.</span><span class="sxs-lookup"><span data-stu-id="99b8e-124">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="99b8e-125">Para este ejemplo, utilizaremos C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="99b8e-125">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="99b8e-126">En c:\MySample, cree `Utf8String.vb` (para el ejemplo de Visual Basic) o `Utf8String.cs` (para el ejemplo de C#) y copie el código muestra de Visual Basic o de C# que corresponda (más abajo) en el archivo.</span><span class="sxs-lookup"><span data-stu-id="99b8e-126">In c:\MySample, create `Utf8String.vb` (for the Visual Basic sample) or `Utf8String.cs` (for the C# sample) and copy the appropriate Visual Basic or C# sample code (below) into the file.</span></span>  
  
4.  <span data-ttu-id="99b8e-127">Compile el código muestra desde el símbolo del sistema ejecutando uno de los comandos siguientes, dependiendo de su opción de lenguaje.</span><span class="sxs-lookup"><span data-stu-id="99b8e-127">Compile the sample code from the command line prompt by executing one of the following, depending on your choice of language.</span></span>  
...
7.  <span data-ttu-id="99b8e-128">Copie el código de instalación de [!INCLUDE[tsql](../../includes/tsql-md.md)] en un archivo y guárdelo como `Install.sql` en el directorio de ejemplo.</span><span class="sxs-lookup"><span data-stu-id="99b8e-128">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
8.  <span data-ttu-id="99b8e-129">Si el ejemplo está instalado en un directorio distinto de `C:\MySample\`, edite el archivo `Install.sql` del archivo como se indica para señalar esa ubicación.</span><span class="sxs-lookup"><span data-stu-id="99b8e-129">If the sample is installed in a directory other then `C:\MySample\`, edit the file `Install.sql` as indicated to point to that location.</span></span>  
  
9. <span data-ttu-id="99b8e-130">Implemente el ensamblado y el procedimiento almacenado ejecutando</span><span class="sxs-lookup"><span data-stu-id="99b8e-130">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql`  
  
10. <span data-ttu-id="99b8e-131">Copie el script de comando de prueba de [!INCLUDE[tsql](../../includes/tsql-md.md)] en un archivo y guárdelo como `test.sql` en el directorio de ejemplo.</span><span class="sxs-lookup"><span data-stu-id="99b8e-131">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
11. <span data-ttu-id="99b8e-132">Ejecute el script de prueba con el siguiente comando</span><span class="sxs-lookup"><span data-stu-id="99b8e-132">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
12. <span data-ttu-id="99b8e-133">Copie el script de limpieza de [!INCLUDE[tsql](../../includes/tsql-md.md)] en un archivo y guárdelo como `cleanup.sql` en el directorio de ejemplo.</span><span class="sxs-lookup"><span data-stu-id="99b8e-133">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
13. <span data-ttu-id="99b8e-134">Ejecute el script con el siguiente comando</span><span class="sxs-lookup"><span data-stu-id="99b8e-134">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="99b8e-135">Código de ejemplo</span><span class="sxs-lookup"><span data-stu-id="99b8e-135">Sample Code</span></span>  
 <span data-ttu-id="99b8e-136">A continuación se muestran las listas de código para este ejemplo.</span><span class="sxs-lookup"><span data-stu-id="99b8e-136">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="99b8e-137">C#</span><span class="sxs-lookup"><span data-stu-id="99b8e-137">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="99b8e-138">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="99b8e-138">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="99b8e-139">Este es el script (`Install.sql`) de instalación de [!INCLUDE[tsql](../../includes/tsql-md.md)], que implementa el ensamblado y crea el UDT en la base de datos.</span><span class="sxs-lookup"><span data-stu-id="99b8e-139">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the UDT in the database.</span></span>  
...
```  
 ...
 <span data-ttu-id="99b8e-140">Este es `test.sql`, que prueba el ejemplo ejerciendo el tipo.</span><span class="sxs-lookup"><span data-stu-id="99b8e-140">This is `test.sql`, which tests the sample by exercising the type.</span></span>  
...
```  
 ...
 <span data-ttu-id="99b8e-141">El siguiente código de [!INCLUDE[tsql](../../includes/tsql-md.md)] quita el ensamblado y el tipo de la base de datos.</span><span class="sxs-lookup"><span data-stu-id="99b8e-141">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly and type from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="99b8e-142">Consulte también</span><span class="sxs-lookup"><span data-stu-id="99b8e-142">See Also</span></span>  
 [<span data-ttu-id="99b8e-143">Escenarios de uso y ejemplos para la integración de Common Language Runtime &#40;CLR&#41;</span><span class="sxs-lookup"><span data-stu-id="99b8e-143">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
