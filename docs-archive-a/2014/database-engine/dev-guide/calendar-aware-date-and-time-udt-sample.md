...
3.  En c:\MySample, cree `CalendarAware.cs` y copie en el archivo el código de ejemplo C# (más abajo).  
  
4.  En c:\MySample, cree el archivo `calendars.txt` y copie el código de ejemplo en el archivo.  
  
5.  En c:\MySample, cree el archivo `calendars.ar-SA.txt` y copie lo siguiente en el archivo:  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 1`  
  
6.  En c:\MySample, cree el archivo `calendars.ja.txt` y copie lo siguiente en el archivo:  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 3`  
  
7.  En c:\MySample, cree el archivo `calendars.``zh-CN.txt` y copie lo siguiente en el archivo:  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 8`  
  
8.  En c:\MySample, cree el archivo `build.com` y copie lo siguiente en el archivo:  
...
9. Compile los ensamblados satélite ejecutando la compilación de archivo en el símbolo del sistema.  
  
10. Compile el código muestra en el símbolo del sistema ejecutando lo siguiente:  
...
11. Copie el código de instalación de [!INCLUDE[tsql](../../includes/tsql-md.md)] en un archivo y guárdelo como `Install.sql` en el directorio de ejemplo.  
  
12. Si el ejemplo está instalado en un directorio distinto de `C:\MySample\`, edite el archivo `Install.sql` del archivo como se indica para señalar esa ubicación.  
  
13. Implemente el ensamblado y el procedimiento almacenado ejecutando  
  
    -   `sqlcmd -E -I -i install.sql`  
  
14. Copie el script de comando de prueba de [!INCLUDE[tsql](../../includes/tsql-md.md)] en un archivo y guárdelo como `test.sql` en el directorio de ejemplo.  
  
15. Ejecute el script de prueba con el siguiente comando  
  
    -   `sqlcmd -E -I -i test.sql`  
  
16. Copie el script de limpieza de [!INCLUDE[tsql](../../includes/tsql-md.md)] en un archivo y guárdelo como `cleanup.sql` en el directorio de ejemplo.  
  
17. Ejecute el script con el siguiente comando  
...
```  
 ...
 Este es el archivo `calendars.txt` necesario para definir los calendarios.  
...
```  
 ...
 Este es el script de instalación de [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`), que implementa los ensamblados y crea las funciones de conversión dentro de la base de datos; también crea una tabla de datos de ejemplo.  
...
```  
 ...
 Este es `test.sql`, que prueba el ejemplo ejecutando las funciones en varios calendarios.  
...
```  
 ...
 El siguiente [!INCLUDE[tsql](../../includes/tsql-md.md)] quita los ensamblados, funciones y tabla de la base de datos.  
