...
|`Connect Timeout`|Determina el tiempo máximo (en segundos) que el cliente intenta una conexión antes de que se agote el tiempo de espera. Si una conexión no se realiza correctamente dentro de este período, el cliente se cierra intentando conectarse y genera un error.|  
...
|`Optimize Response`|Máscara de bits que indica cuáles de las siguientes optimizaciones de respuesta de consulta están habilitadas.<br /><br /> 0x01: valor predeterminado. Usar el NormalTupleSet <br />0x02: usar cuando las segmentaciones están vacías|  
