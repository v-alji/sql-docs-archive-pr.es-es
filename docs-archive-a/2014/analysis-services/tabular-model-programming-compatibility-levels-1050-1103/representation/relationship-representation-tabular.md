...
ms.locfileid: "87745418"
---
# <a name="relationship-representation-tabular"></a><span data-ttu-id="7c96e-102">Representación de relaciones (tabular)</span><span class="sxs-lookup"><span data-stu-id="7c96e-102">Relationship Representation (Tabular)</span></span>
  <span data-ttu-id="7c96e-103">Una relación es una conexión entre dos tablas de datos.</span><span class="sxs-lookup"><span data-stu-id="7c96e-103">A relationship is a connection between two tables of data.</span></span> <span data-ttu-id="7c96e-104">La relación establece cómo se deben relacionar los datos de las dos tablas.</span><span class="sxs-lookup"><span data-stu-id="7c96e-104">The relationship establishes how the data in the two tables should be correlated.</span></span>  
  
 <span data-ttu-id="7c96e-105">Vea [Relationship Representation (Tabular)](relationship-representation-tabular.md) para obtener una explicación detallada acerca de cómo crear y manipular la representación de la relación.</span><span class="sxs-lookup"><span data-stu-id="7c96e-105">See [Relationship Representation (Tabular)](relationship-representation-tabular.md) for a detailed explanation on how to create and manipulate the relationship representation.</span></span>  
  
## <a name="relationship-representation"></a><span data-ttu-id="7c96e-106">Representación de relaciones</span><span class="sxs-lookup"><span data-stu-id="7c96e-106">Relationship Representation</span></span>  
 <span data-ttu-id="7c96e-107">En los modelos tabulares, se pueden definir varias relaciones entre dos tablas.</span><span class="sxs-lookup"><span data-stu-id="7c96e-107">In tabular models, multiple relationships can be defined between two tables.</span></span> <span data-ttu-id="7c96e-108">Cuando se definen varias relaciones entre dos tablas, solo una puede establecerse como la relación predeterminada del modelo y se denomina relación "activa". Todas las demás relaciones se denominan "inactivas".</span><span class="sxs-lookup"><span data-stu-id="7c96e-108">When multiple relationships, between two tables, are defined only one can be defined as the default relationship for the model and it is named as the Active relationship; all other relationships are named as Inactive.</span></span>  
  
### <a name="relationship-in-amo"></a><span data-ttu-id="7c96e-109">Relación en AMO</span><span class="sxs-lookup"><span data-stu-id="7c96e-109">Relationship in AMO</span></span>  
 <span data-ttu-id="7c96e-110">Por lo que respecta a los objetos AMO, todas las relaciones inactivas tienen una representación de asignación de uno a uno con <xref:Microsoft.AnalysisServices.Relationship> y no se necesita ningún otro objeto AMO principal. En el caso de la relación activa, hay otros requisitos y, además, es necesaria una asignación con <xref:Microsoft.AnalysisServices.ReferenceMeasureGroupDimension>.</span><span class="sxs-lookup"><span data-stu-id="7c96e-110">In terms of AMO objects all inactive relationships have a representation of a one to one mapping relationship with <xref:Microsoft.AnalysisServices.Relationship> and no other main AMO objects are required; for the active relationship other requirements exist and a mapping to the <xref:Microsoft.AnalysisServices.ReferenceMeasureGroupDimension> is also required.</span></span>  
  
 <span data-ttu-id="7c96e-111">En los fragmentos de código siguientes se muestra cómo se crea una relación en modelos tabulares, cómo se activa una relación y cómo se define una clave principal en una tabla (distinta de “RowNumber”).</span><span class="sxs-lookup"><span data-stu-id="7c96e-111">The following code snippets show how to create a relationship in Tabular models, how to activate a relationship and how to define a primary key in a table (other than "RowNumber").</span></span> <span data-ttu-id="7c96e-112">Para crear una relación activa, debe definirse una clave principal en la tabla de clave principal - PKTableName - de la relación (uno de los extremos de la relación). En el ejemplo que se muestra aquí, se crea la clave principal de PKColumnName si no hay ninguna clave principal definida en la columna.</span><span class="sxs-lookup"><span data-stu-id="7c96e-112">To create an active relationship a primary key need to be defined in the primary key table - PKTableName - of the relationship (the one side of the relationship), the sample shown here creates the primary key on the PKColumnName if no primary key is defined in this column.</span></span> <span data-ttu-id="7c96e-113">Se pueden crear relaciones inactivas sin necesidad de tener una clave principal en la columna de clave principal.</span><span class="sxs-lookup"><span data-stu-id="7c96e-113">Inactive relationships can be created without the need of having a primary key on the primary key column.</span></span>  
...
## <a name="amo2tabular-sample"></a><span data-ttu-id="7c96e-114">Ejemplo AMO2Tabular</span><span class="sxs-lookup"><span data-stu-id="7c96e-114">AMO2Tabular sample</span></span>  
 <span data-ttu-id="7c96e-115">Sin embargo, para saber cómo usar AMO para crear y manipular representaciones de relación vea el código fuente del ejemplo AMO a tabular.</span><span class="sxs-lookup"><span data-stu-id="7c96e-115">However, to have an understanding on how to use AMO to create and manipulate relationship representations see the source code of the AMO to Tabular sample.</span></span> <span data-ttu-id="7c96e-116">El ejemplo está disponible en Codeplex.</span><span class="sxs-lookup"><span data-stu-id="7c96e-116">The sample is available at Codeplex.</span></span> <span data-ttu-id="7c96e-117">Nota importante sobre el código: el código se proporciona solo como apoyo de los conceptos lógicos explicados aquí y no debe utilizarse en un entorno de producción; no debe usarse para otros fines excepto el pedagógico.</span><span class="sxs-lookup"><span data-stu-id="7c96e-117">An important note about the code: the code is provided only as a support to the logical concepts explained here and should not be used in a production environment; nor should it be used for other purpose other than the pedagogical one.</span></span>  
