...
ms.locfileid: "87748952"
---
# <a name="calculated-column-representation-tabular"></a><span data-ttu-id="3dfa6-102">Representación de la columna calculada (tabular)</span><span class="sxs-lookup"><span data-stu-id="3dfa6-102">Calculated Column Representation (Tabular)</span></span>
  <span data-ttu-id="3dfa6-103">Una columna calculada es una expresión de DAX que crea una nueva columna en una tabla, en la que se almacenan los valores obtenidos.</span><span class="sxs-lookup"><span data-stu-id="3dfa6-103">A calculated column is DAX expression that creates a new column in a table and the obtained values are stored in the table.</span></span> <span data-ttu-id="3dfa6-104">La expresión de columna calculada se evalúa cada vez que se procesa la tabla.</span><span class="sxs-lookup"><span data-stu-id="3dfa6-104">The calculated column expression is evaluated every time the table is processed.</span></span>  
  
## <a name="calculated-column-representation"></a><span data-ttu-id="3dfa6-105">Representación de la columna calculada</span><span class="sxs-lookup"><span data-stu-id="3dfa6-105">Calculated Column Representation</span></span>  
  
### <a name="calculated-columns-in-amo"></a><span data-ttu-id="3dfa6-106">Columnas calculadas en AMO</span><span class="sxs-lookup"><span data-stu-id="3dfa6-106">Calculated Columns in AMO</span></span>  
 <span data-ttu-id="3dfa6-107">Cuando se usa AMO para administrar una tabla del modelo tabular, no hay ninguna correspondencia uno a uno entre el objeto y columna calculada en AMO.</span><span class="sxs-lookup"><span data-stu-id="3dfa6-107">When using AMO to manage a tabular model table, there is no one-to-one object match for a calculated column in AMO.</span></span> <span data-ttu-id="3dfa6-108">Una columna calculada se representa mediante un atributo en <xref:Microsoft.AnalysisServices.Dimension> y un atributo en <xref:Microsoft.AnalysisServices.MeasureGroup>.</span><span class="sxs-lookup"><span data-stu-id="3dfa6-108">A calculated column is represented by an attribute in <xref:Microsoft.AnalysisServices.Dimension> and an attribute in <xref:Microsoft.AnalysisServices.MeasureGroup>.</span></span>  
  
 <span data-ttu-id="3dfa6-109">En el fragmento de código siguiente se muestra cómo se agrega una columna calculada a un modelo tabular existente.</span><span class="sxs-lookup"><span data-stu-id="3dfa6-109">The following code snippet shows how to add a calculated column to an existing tabular model.</span></span> <span data-ttu-id="3dfa6-110">En el código se presupone que tiene un objeto de base de datos de AMO, newDatabase, y un objeto de cubo de AMO, modelCube.</span><span class="sxs-lookup"><span data-stu-id="3dfa6-110">The code assumes you have an AMO database object, newDatabase, and an AMO cube object, modelCube.</span></span>  
