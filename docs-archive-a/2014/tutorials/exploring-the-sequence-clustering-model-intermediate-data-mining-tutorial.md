...
1.  Haga clic con el botón derecho `Pacific Cluster` , seleccione obtener **detalles**y, a continuación, seleccione **columnas de modelo y estructura**.  
...
1.  Haga clic con el botón derecho en la fila del **modelo** en la columna del clúster `Largest Cluster` y seleccione **Mostrar leyenda**.  
...
2.  Compare los histogramas de los clústeres a los que ha cambiado el nombre `Pacific Cluster` y `Largest Cluster` . Cada clúster aparece en una columna diferente.  
  
     En ambos se usan colores sólidos, pero los colores son diferentes.  
  
3.  En la `Region` fila, coloque el mouse sobre el histograma de color para `Largest Cluster` .  
  
     En la Información sobre herramientas se muestran los porcentajes reales de casos de cada región.  
  
4.  Haga clic con el botón secundario en el histograma de color de la `Region` fila de `Pacific Cluster` , seleccione obtener **detalles**y, a continuación, seleccione **solo columnas del modelo**.  
...
7.  En la lista desplegable **clúster** , seleccione **población (todos)**.  
 ...
 La segunda es que hay un efecto de agrupación en clústeres fuerte en los atributos de `Region` , lo que hace más difícil ver los grupos de secuencias. Por tanto, decídase a crear otro modelo que use exclusivamente secuencias y no incluya las columnas de las regiones o los ingresos.  
