...
11. Seleccione la **clave**, la **entrada**y las `Predict` casillas como se muestra en la tabla siguiente:  
    ...
    |Orders (Pedidos)|Entrada/Predicción|  
...
14. En la página **finalización del asistente** , en **nombre**de la estructura de minería de datos, escriba `Call Center` .  
  
15. En el **nombre del modelo de minería de datos**, escriba `Call Center Default NN` y, a continuación, haga clic en **Finalizar**.  
...
-   **Discretizar mediante áreas iguales**  
 ...
 Como alternativa, en lugar de usar los valores numéricos, podría agregar una columna derivada independiente que clasifique los grados de servicio en intervalos de destino predefinidos, como el **mejor** (ServiceGrade \<= 0.05), **Acceptable** (0.10 > ServiceGrade > 0,05) y **malo** (ServiceGrade >= 0,10).  
...
8.  Cambie el valor de la propiedad `Content` de `Continuous` a `Discretized`.  
    ...
    |`DiscretizationBucketCount`|No hay ningún valor|4|  
...
5.  Haga clic en cualquier parte de la pestaña **modelo de minería de datos** .  
    ...
    |Orders (Pedidos)|Entrada|Entrada|  
    |ServiceGrade Binned|Ignorar|Predicción (ServiceGrade)|  
...
2.  En la fila del parámetro HOLDOUT_SEED, haga clic en la celda vacía situada debajo de **valor**y escriba `1` . Haga clic en **OK**. Repita este paso para cada modelo asociado a la estructura.  
