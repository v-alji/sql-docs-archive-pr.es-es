...
>  `sp_xml_preparedocument`usa una versión actualizada de SQL del analizador de MSXML Msxmlsql.dll. Esta versión del analizador de MSXML se diseñó para admitir [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] y seguir siendo compatible con versiones anteriores con MSXML versión 2.6.
 ...
 ![Análisis de XML con OPENXML](../../database-engine/media/xmlsp.gif "Análisis de XML con OPENXML")
...
#### <a name="using-the-edge-table-format"></a>Utilizar el formato de tabla irregular
 ...
 Las tablas irregulares representan, en una sola tabla, la estructura exacta del documento XML. Esta estructura incluye los nombres de elementos y atributos, la jerarquía del documento, los espacios de nombres y las instrucciones de procesamiento. El formato de tabla irregular permite obtener información adicional que no se muestra a través de las metapropiedades. Para obtener más información acerca de las metapropiedades, vea [Specify Metaproperties in OPENXML](../xml/specify-metaproperties-in-openxml.md).
