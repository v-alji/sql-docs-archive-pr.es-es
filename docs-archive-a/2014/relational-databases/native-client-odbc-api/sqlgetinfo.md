...
|SQL_CONVERT_DATE|No se admite la conversión del tipo de datos SQL_TYPE_DATE de ODBC. El [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] controlador ODBC de Native Client admite el [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] tipo de datos **DateTime** como SQL_TYPE_TIMESTAMP de tipo ODBC. Vea SQL_CONVERT_TIMESTAMP a continuación.|  
...
|SQL_CONVERT_INTERVAL_YEAR_MONTH|No se admite la conversión de tipos de datos interval.|  
|SQL_CONVERT_INTERVAL_DAY_TIME|No se admite la conversión de tipos de datos interval.|  
...
|SQL_DATA_SOURCE_NAME|Nombre del origen de datos actual. Establece el valor al que apunta *StringLengthPtr* en 0 si la conexión no especifica un nombre de origen de datos.|  
|SQL_DATA_SOURCE_READ_ONLY|Depende de la configuración del atributo de conexión SQL_ATTR_ACCESS_MODE.|  
|SQL_DATABASE_NAME|Base de datos actual de la conexión.|  
|SQL_DBMS_NAME|"Microsoft [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]"|  
|SQL_DBMS_VER|Número de versión de la instancia conectada de [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].|  
...
|SQL_IDENTIFIER_CASE|SQL_IC_MIXED si está conectado a un servidor que ejecuta un criterio de ordenación sin distinción de mayúsculas y minúsculas.<br /><br /> SQL_IC_SENSITIVE si está conectado a un servidor que ejecuta un criterio de ordenación con distinción de mayúsculas y minúsculas.|  
...
|SQL_ODBC_VER|Número de versión actual del administrador de controladores ODBC.|  
...
|SQL_QUOTED_IDENTIFIER_CASE|SQL_IC_MIXED cuando está conectado a un servidor que ejecuta un criterio de ordenación sin distinción de mayúsculas y minúsculas.<br /><br /> SQL_IC_SENSITIVE cuando está conectado a un servidor que ejecuta un criterio de ordenación con distinción de mayúsculas y minúsculas.|  
|SQL_ROW_UPDATES|"N"|  
